...
export const checkApiStatus = (): AppThunk => {
    return async (dispatch) => {
        let response = await fetch("http://localhost:8080/api/v1/ping")
        if (!response.ok) {
            dispatch(apiOff());
        }
...
export const checkDockerStatus = (): AppThunk => {
    return async (dispatch) => {
        let response = await fetch('http://localhost:8080/api/v1/status');
...
export const { dockerReady, dockerDegraded, dockerOff, apiOn, apiDegraded, apiOff } = homeSlice.actions;
